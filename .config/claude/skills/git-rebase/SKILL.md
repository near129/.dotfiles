---
name: git-rebase
description: Rebase the current branch onto another branch's latest changes. Shows incoming changes before rebasing and verifies both the original and incoming changes are reflected afterward.
argument-hint: [target-branch]
allowed-tools: Bash(git status:*), Bash(git branch:*), Bash(git fetch:*), Bash(git log:*), Bash(git diff:*), Bash(git rev-list:*), Bash(git rev-parse:*), Bash(git symbolic-ref:*), Bash(git rebase:*), Bash(git stash:*), Bash(git add:*), Bash(git push:*), Read
---

Rebase the current branch onto the latest state of another branch, replaying the current branch's commits on top.
...
1. **Validate preconditions**
   ...
   - Check if a rebase is already in progress: `git status` mentioning "rebase in progress" means the user must resolve or abort it first. Inform the user and stop.
...
5. **Run the rebase**
   - Run `git rebase origin/<target>`.
   - If the rebase fails due to conflicts, go to step 6. Otherwise, go to step 7.
...
6. **Handle conflicts (if any)**
   ...
   - If abort: run `git rebase --abort` and inform the user.
   - If resolve: help the user fix each conflict, stage resolved files with `git add <file>`, then run `git rebase --continue`. Repeat for each commit if multiple conflicts arise.

7. **Verify the result**
   - Run `git log --oneline -10` to confirm the current branch history. The user's commits should appear on top of the target branch's commits (linear history, no merge commit).
   ...
   - Report: method used (rebase), target branch, number of commits integrated, and current HEAD.
...
8. **Check if incoming changes require follow-up work on this branch**
   - Regardless of whether conflicts occurred, review the incoming changes with `git log --oneline ORIG_HEAD..origin/<target>` and the full diff with `git diff ORIG_HEAD...origin/<target>` (ORIG_HEAD points to the branch tip before the rebase).
...
9. **Push (if needed)**
   - Since rebase rewrites history, `git push --force-with-lease` is required for branches already pushed to the remote. Ask the user if they want to push: if yes, run `git push --force-with-lease origin <current-branch>`.
...
- Never run `git push --force` (without `--force-with-lease`); always use `--force-with-lease` to protect against overwriting others' work.
